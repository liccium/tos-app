...
1.2 With Liccium Services, professional content creators and rightsholders can generate and manage content-derived identifiers (ISCC codes), provide and bind product metadata, rights and licences to the ISCC codes, and make public declarations using verifiable credentials (VCs) to provide attribution for the declaration.
...
Liccium Services are using software to generate content-derived identifiers, based on the International Standard Content Code (ISCC), which is published by ISO under ISO 24138:2024. ISCC is a content-derived identifier and identification system for digital media assets (including encodings of text, images, audio, video), using cryptographic and similarity-preserving hashes to create a unique ID for each digital asset.
